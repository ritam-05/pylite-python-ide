...
class Parser:
    ...
    def statement(self) -> ASTNode:
        ...
        if self.current_token().type == TokenType.RETURN:
            ...
            if self.current_token().type in (TokenType.NEWLINE, TokenType.EOF, TokenType.DEDENT):
                return Return(value=NoneVal())
            exprs = [self.expression()]
            while self.current_token().type == TokenType.COMMA:
                self.eat(TokenType.COMMA)
                if self.current_token().type in (TokenType.NEWLINE, TokenType.EOF, TokenType.DEDENT): break
        ...
        if self.current_token().type in assign_tokens:
            ...
            while self.current_token().type == TokenType.COMMA:
                self.eat(TokenType.COMMA)
                if self.current_token().type in (TokenType.NEWLINE, TokenType.EOF): break
    ...
    def logic_or(self) -> ASTNode:
        ...
        while self.current_token().type == TokenType.OR:
            op = self.current_token().value; self.eat(TokenType.OR)
    ...
    def logic_and(self) -> ASTNode:
        ...
        while self.current_token().type == TokenType.AND:
            op = self.current_token().value; self.eat(TokenType.AND)
            node = LogicalOp(left=node, op=op, right=self.logic_not())
        return node

    def logic_not(self) -> ASTNode:
        if self.current_token().type == TokenType.NOT:
            op = self.current_token().value; self.eat(TokenType.NOT)
            return UnaryOp(op=op, operand=self.logic_not())
        return self.comparison()

    # MODIFIED: is and is not
    def comparison(self) -> ASTNode:
        node = self.expr()
        while self.current_token().type in (TokenType.EQ, TokenType.NEQ, TokenType.LT, TokenType.GT, TokenType.IS):
    ...
    def power(self) -> ASTNode:
        ...
        if self.current_token().type == TokenType.DOUBLE_STAR:
            op = self.current_token().value; self.eat(TokenType.DOUBLE_STAR)
            node = BinOp(left=node, op=op, right=self.unary()) 
        return node
    ...
    def factor(self) -> ASTNode:
        ...
        if token.type == TokenType.NUMBER:
            ...
            self.eat(TokenType.STRING); node = String(value=token.value[1:-1])
        elif token.type == TokenType.TRUE:
            self.eat(TokenType.TRUE); node = Boolean(value=True)
        elif token.type == TokenType.FALSE:
            self.eat(TokenType.FALSE); node = Boolean(value=False)
        elif token.type == TokenType.NONE:
            self.eat(TokenType.NONE); node = NoneVal() # ADDED
        elif token.type == TokenType.SUPER:
            self.eat(TokenType.SUPER); self.eat(TokenType.LPAREN); self.eat(TokenType.RPAREN); node = Super()
        elif token.type == TokenType.IDENTIFIER:
            self.eat(TokenType.IDENTIFIER); node = Name(value=token.value)
        elif token.type == TokenType.LPAREN:
            self.eat(TokenType.LPAREN)
            if self.current_token().type == TokenType.RPAREN:
                self.eat(TokenType.RPAREN); node = TupleLiteral(elements=[])
            else:
                elements = [self.expression()]
                is_tuple = False
                while self.current_token().type == TokenType.COMMA:
                    is_tuple = True; self.eat(TokenType.COMMA)
                    if self.current_token().type != TokenType.RPAREN: elements.append(self.expression())
                self.eat(TokenType.RPAREN)
            ...
            if self.current_token().type == TokenType.RBRACKET:
                self.eat(TokenType.RBRACKET); node = ListLiteral(elements=[])
            else:
                ...
                if self.current_token().type == TokenType.FOR:
                    ...
                    while self.current_token().type == TokenType.IF:
                        self.eat(TokenType.IF); ifs.append(self.expression())
                    ...
                    self.eat(TokenType.RBRACKET); node = ListLiteral(elements=elements)
            
        elif token.type == TokenType.LBRACE:
            self.eat(TokenType.LBRACE)
            if self.current_token().type == TokenType.RBRACE:
                self.eat(TokenType.RBRACE); node = DictLiteral(keys=[], values=[])
            else:
                ...
                if self.current_token().type == TokenType.FOR:
                    ...
                    keys = [first_key]; values = [first_val]
                    while self.current_token().type == TokenType.COMMA:
                        self.eat(TokenType.COMMA)
                    ...
                    self.eat(TokenType.RBRACE); node = DictLiteral(keys=keys, values=values)
        else:
        ...
        while self.current_token().type in (TokenType.LPAREN, TokenType.LBRACKET, TokenType.DOT):
            if self.current_token().type == TokenType.LPAREN:
                ...
                if self.current_token().type == TokenType.COLON:
                    is_slice = True; self.eat(TokenType.COLON)
